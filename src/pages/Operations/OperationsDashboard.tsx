...
const OperationsDashboard: React.FC = () => {
    ...
    const fetchAllShipments = async () => {
      try {
        ...
        if (Array.isArray(data)) {
          ...
          setShipments([...data.shipments].reverse());
        } else {
          console.warn("Backend sent weird data:", data);
          setShipments([]);
        }

      } catch (error) {
        console.error("Failed to fetch all shipments", error);
        setFetchError('Could not load shipments from the server.');
      } finally {
        setLoading(false);
      }
    };
  ...
  if (loading) return <div className="p-6 text-gray-500 font-medium">Loading Operations Control Tower...</div>;
  
  if (fetchError) return <div className="p-6 text-red-500 font-medium">{fetchError}</div>;
            ...
            <tbody className="bg-white divide-y divide-gray-200">
              {filteredShipments.length === 0 ? (
                ...
                filteredShipments.map((shipment) => (
                  <tr key={shipment?.id || Math.random()} className="hover:bg-gray-50 transition">
                    <td className="px-6 py-4 whitespace-nowrap text-sm font-medium text-gray-900">
                      {shipment?.shipment_code || 'Pending'}
                    </td>
                    <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                      <span className="font-semibold text-gray-700">{shipment?.pickup_city || 'N/A'}</span> &rarr; <span className="font-semibold text-gray-700">{shipment?.receiver_city || 'N/A'}</span>
                    </td>
                    ...
                    <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium space-x-3">
                      ...
                      <Link 
                        to={`/operations/track/${shipment?.id}`} 
                        className="text-[#1B2A4A] hover:text-[#D97706] bg-gray-100 px-3 py-1.5 rounded border border-gray-200 inline-block"
                      >
                        Track
                      </Link>
